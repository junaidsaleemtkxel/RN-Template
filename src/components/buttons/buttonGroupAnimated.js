...
export default function AnimatedGroupButton({
  ...
  if (initalIndex < data.length) {
    ...
    if (
      !disableAutoSwipe &&
      selectedTabIndex != initalIndex &&
      data[initalIndex].x >= 0
    ) {
                ...
                <Wrapper flexDirectionRow>
                  {!hideLabel ? (
                    <Text
                      ...
                      {indentifier
                        ? data[selectedTabIndex][indentifier]
                        : data[selectedTabIndex].label}
                    </Text>
                  ) : null}
